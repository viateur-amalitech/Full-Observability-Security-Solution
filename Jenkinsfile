pipeline {
    ...
    parameters {
        string(name: 'DOCKER_HUB_USER', defaultValue: '', description: 'Docker Hub Username')
        ...
        stage('Docker Build') {
            steps {
                script {
                    ...
                    sh "docker build --build-arg APP_VERSION=${params.APP_VERSION} -t ${DOCKER_IMAGE}:${params.APP_VERSION} app/"
